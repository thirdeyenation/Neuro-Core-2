...
- Compose capabilities in `NeuroCore2Service` (capture, retrieve, validate, store, activity).
- Keep all Agent Zero integration under `plugins/neuro_core_2/`.

## Consequences

- Core modules (`neuro_core_2.py`, `memory_lifecycle.py`, `memory_store.py`, `sqlite_store.py`, `activity_ledger.py`, `neuro_core_2_service.py`) remain host-independent.
